...
# Lightweight VLM architecture (<1B parameters)
...
# Custom Dataset for MedPix 2.0 with caching
class MedPixDataset(Dataset):
    def __init__(self, data_jsonl_files, desc_jsonl_files, image_dir, tokenizer, processor, cache_dir='cache'):
        ...
        self.location_map = {'Abdomen': 0, 'Head': 1, 'Reproductive and Urinary System': 2, 'Thorax': 3, 'Spine and Muscles': 4}
...
class DistillationLoss(nn.Module):
    ...
    def forward(self, student_logits, teacher_logits, labels):
        ...
        loss_modality = self.ce_loss(student_modality, labels[0]) + \
                        self.alpha * self.kl_div(
                            torch.log_softmax(student_modality / self.temperature, dim=1),
                            torch.softmax(teacher_modality / self.temperature, dim=1)
                        )
        loss_location = self.ce_loss(student_location, labels[1]) + \
                        self.alpha * self.kl_div(
...
class DistillationLoss(nn.Module):
    ...
    def forward(self, student_logits, teacher_logits, labels):
        ...
        def kd_loss(student, teacher, label):
            ce = self.ce_loss(student, label)
            kd = self.kl_div(
                torch.log_softmax(student / self.temperature, dim=1),
                torch.softmax(teacher / self.temperature, dim=1)
            ) * (self.temperature ** 2)
            return (1 - self.alpha) * ce + self.alpha * kd

        loss_modality = kd_loss(student_modality, teacher_modality, labels[0])
        loss_location = kd_loss(student_location, teacher_location, labels[1])
...
def train_and_optimize(model, teacher_model, train_loader, val_loader, epochs=10, accum_steps=4):
    ...
    # Freeze early layers for first 5 epochs
    ...
    for epoch in range(epochs):
        ...
        for i, (images, texts, modality_labels, location_labels) in enumerate(train_loader):
            ...
            with autocast('cuda:3'):
                student_modality, student_location = model(images, texts)
                loss = criterion((student_modality, student_location),
                               (teacher_modality, teacher_location),
        ...
        with open(log_file, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([epoch + 1, total_loss / len(train_loader),
                           modality_acc, location_acc,
                           modality_metrics[0], modality_metrics[1], modality_metrics[2],
                           location_metrics[0], location_metrics[1], location_metrics[2]])

        # Save confusion matrices
        np.savetxt(f'logs/confusion_modality_epoch_{epoch+1}.csv',
                  confusion_matrix(modality_labels_all, modality_preds),
                  delimiter=',', fmt='%d')
        np.savetxt(f'logs/confusion_location_epoch_{epoch+1}.csv',
                  confusion_matrix(location_labels_all, location_preds),
                  delimiter=',', fmt='%d')

        print(f'Epoch {epoch+1}: Loss: {total_loss/len(train_loader):.4f}, '
              f'Modality Acc: {modality_acc:.4f}, Location Acc: {location_acc:.4f}')
...
if __name__ == '__main__':
    ...
    torch.save(teacher_model.state_dict(), 'results/teacher_model.pth')

    # Initialize dataset and dataloaders
    train_dataset = MedPixDataset(
        'MedPix-2-0/splitted_dataset/data_train.jsonl',
        'MedPix-2-0/splitted_dataset/descriptions_train.jsonl',
        'MedPix-2-0/images',
        tokenizer,
        processor
    )
    val_dataset = MedPixDataset(
        'MedPix-2-0/splitted_dataset/data_dev.jsonl',
        'MedPix-2-0/splitted_dataset/descriptions_dev.jsonl',
        'MedPix-2-0/images',
        tokenizer,
        processor
    )
    ...
    # Initialize and train lightweight model
    model = LightweightVLM()
    print("Student Model parameters:",
      sum(p.numel() for p in model.parameters()))
    torch.save(model.state_dict(), 'results/student_model.pth')

    optimized_model = train_and_optimize(model, teacher_model, train_loader, val_loader)
    print("Optimized Student Model parameters:",
      sum(p.numel() for p in optimized_model.parameters()))
    torch.save(optimized_model.state_dict(), 'results/optimized_student_model.pth')
